...
namespace appLauncher.Core.Converters
{
    ...
    public class EditModeToIconConverter : IValueConverter
    {
        ...
        /// <returns>A SymbolIcon representing 'Save' if true, or 'Edit' if false.</returns>
        public object Convert(object value, Type targetType, object parameter, string language)
        {
            if (value is bool isEditMode)
            {
                return isEditMode ? new SymbolIcon(Symbol.Save) : new SymbolIcon(Symbol.Edit);
            }
            // Return a default icon if the value is not a boolean or is null
